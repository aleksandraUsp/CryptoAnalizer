...
public class Encoder implements Action {
    ...
    public Result execute(String[] parameters) {  //parameters = [text.txt encode.txt 12]
        ...
        if ((Path.of(parameters[0])).isAbsolute()) {pathIn = Path.of(parameters[0]);}
        else if (!(Path.of(parameters[0])).isAbsolute()) {pathIn = Path.of(Constants.TXT_FOLDER + parameters[0]);}
        ...
        if ((Path.of(parameters[1])).isAbsolute()) {pathOut = Path.of(parameters[1]);}
        else if (!(Path.of(parameters[1])).isAbsolute()) {pathOut = Path.of(Constants.TXT_FOLDER + parameters[1]);}
        else pathOut =Path.of(Constants.TXT_FOLDER +"encode.txt");
            if (!Files.exists(pathOut))
                try { Files.createFile(pathOut);
    ...
    public TreeMap<Character, Character> getTableEncodeChar(int key) {
